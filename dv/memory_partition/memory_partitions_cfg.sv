...
class memory_partitions_cfg #(int addr_width=32) extends uvm_component;
   ...
   rand bit [addr_width-1:0]  m_base_addr;
   // ------------------------------------------
   ...
   protected int req_size_list[bit[addr_width -1:0]];
    ...
    `uvm_component_param_utils_begin( memory_partitions_cfg #(addr_width) )
       ...
       `uvm_field_int(  m_base_addr,                   UVM_DEFAULT | UVM_HEX)
   ...
   constraint c_addr0  { m_low_addr[0] == m_base_addr;}
   constraint c_addr1  { foreach (m_low_addr[i]) m_high_addr[i] - m_low_addr[i] == m_partition_size[i] - 1;}
   constraint c_addr2  { m_high_addr[m_num_partitions -1] - m_low_addr[0] == m_mem_size - 1;}
